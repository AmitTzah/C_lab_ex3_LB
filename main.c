...
void receives_message_to_full_request_until_num_of_request_ends(
        int number_of_requests_ends, int accept_client_socket, char **full_request,
        size_t *size_of_full_request) {
    char receive_buffer[MAX_LEN_RECV];

    do {
        size_t bytes_recv =
                recv(accept_client_socket, receive_buffer, MAX_LEN_RECV, 0);
        ...
        *full_request = (char *)realloc(*full_request, *size_of_full_request);
        ...
        char *pointer_to_start_of_current_message =
                *full_request + old_size_of_full_request;

        memcpy((pointer_to_start_of_current_message), receive_buffer, bytes_recv);

    } while (return_number_of_times_request_end_in_request(
...
int main() {
    ...
    int port_server, port_client, servers_accept_sockets_array[3],
    ...
    while (bind(main_server_socket, (struct sockaddr *)&service_server,
                sizeof(service_server)) == -1) {
        port_server = ranged_rand(MIN_PORT_VAL, MAX_PORT_VAL);
        service_server.sin_port = htons(port_server);
    }
    ...
    while (bind(main_client_socket, (struct sockaddr *)&service_client,
                sizeof(service_client)) == -1) {
        port_client = ranged_rand(MIN_PORT_VAL, MAX_PORT_VAL);
        service_client.sin_port = htons(port_client);
    }
    ...
    while (i < NUMBER_OF_SERVERS) {
        servers_accept_sockets_array[i] = accept(
                main_server_socket, (struct sockaddr *)&service_server, &addr_len);
    ...
    while (1) {
        ...
        accept_client_socket = accept(
                main_client_socket, (struct sockaddr *)&service_client, &addr_len);
        ...
        receives_message_to_full_request_until_num_of_request_ends(
                1, accept_client_socket, &full_request, &size_of_full_request);

        // sends the string to next server by order
        send(servers_accept_sockets_array[i], full_request, size_of_full_request,
             0);
        size_of_full_request = 0;
        free(full_request);
        full_request = malloc(sizeof(char));
        receives_message_to_full_request_until_num_of_request_ends(
                2, servers_accept_sockets_array[i], &full_request,
